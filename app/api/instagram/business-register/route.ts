...
async function getInstagramBusinessInfo(accessToken: string, userId: string) {
  try {
    const response = await fetch(
      `https://graph.instagram.com/${userId}?fields=id,username,name,biography,followers_count,follows_count,media_count,profile_picture_url,website&access_token=${accessToken}`
    )
